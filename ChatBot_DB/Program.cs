...
namespace ChatBot_DB
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            BinDB binDB = new() { SushiTableId = sushisDB.TableId };
            OrderDB orderDB = new() { SushiTableId = sushisDB.TableId };
            OrdersDB ordersDB = new();
            sushiRacksDB.CreateTable(Guid.NewGuid());
            binDB.CreateTable(Guid.NewGuid());
            orderDB.CreateTable(Guid.NewGuid());
            ordersDB.CreateTable(Guid.NewGuid());

            usersDB.CreateItem(new() { Name = "Вася", Password = "123456", Mail = "vasya@a.aa", 
                UserID = Guid.NewGuid(),
                OrdersTableId = Guid.NewGuid(),
                BinTableId = Guid.NewGuid()
        });
            usersDB.CreateItem(new() { Name = "Петя", Password = "123456", Mail = "petya@a.aa",
                UserID = Guid.NewGuid(),
                OrdersTableId = Guid.NewGuid(),
                BinTableId = Guid.NewGuid()
            });
            usersDB.CreateItem(new() { Name = "Миша", Password = "123456", Mail = "misha@a.aa",
                UserID = Guid.NewGuid(),
                OrdersTableId = Guid.NewGuid(),
                BinTableId = Guid.NewGuid()
            });
            usersDB.CreateItem(new() { Name = "Саша", Password = "123456", Mail = "sasha@a.aa",
                UserID = Guid.NewGuid(),
                OrdersTableId = Guid.NewGuid(),
                BinTableId = Guid.NewGuid()
            });
